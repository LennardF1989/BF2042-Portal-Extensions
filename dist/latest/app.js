...
BF2042Portal.Extensions = (function () {
    const BlocklyWrapper = (function (blockly, blocklyTranslations) {
        ...
        this.ContextMenu = {
            ...
            getOptionsForWorkspace: function (workspace) {
                return blockly.ContextMenuRegistry.registry.getContextMenuOptions(self.ContextMenu.ScopeType.WORKSPACE, {
                    workspace: workspace,
                });
            },
            getOptionsForBlock: function (block) {
                return blockly.ContextMenuRegistry.registry.getContextMenuOptions(self.ContextMenu.ScopeType.BLOCK, {
                    block: block,
                });
            },
            getAllItems: function () {
                return [...blockly.ContextMenuRegistry.registry.registry_].map((e) => {
                    return {
                        key: e[0],
                        value: e[1],
                    };
        ...
        this.Xml = {
            ...
            blockToXml: function (block) {
                ...
                const xmlText = blockly.Xml.domToText(xmlDom).replace('xmlns="https://developers.google.com/blockly/xml"', "");
                return xmlText;
            },
        };
        return this;
    ...
    function exportBlocksWorkspace() {
        return exportBlocks("exportBlocksWorkspace", BlocklyWrapper.ContextMenu.ScopeType.WORKSPACE);
    }
    function exportBlocksBlock() {
        return exportBlocks("exportBlocksBlock", BlocklyWrapper.ContextMenu.ScopeType.BLOCK);
    }
    function exportBlocks(id, scopeType) {
        ...
        async function blocksToSvg(blocks, convertImagesToDataURI) {
            ...
            svg.setAttribute("class", `blocklySvg ${workspace.options.renderer || "geras"}-renderer ${workspace.getTheme
            ...
            if (convertImagesToDataURI) {
                ...
                for (const match of matches) {
                    ...
                    try {
                        let dataUrl = await new Promise(async (resolve, reject) => {
                            try {
                                //TODO: Implement a time-out mechanism
                                const response = await fetch("https://portal.battlefield.com" + url);
            ...
            svgAsXML = svgAsXML.replace(/xlink:href=\"(.*?)\"/g, function (_, url) {
                    ...
                    callback: function () {
                        ...
                        for (let i2 = 0; i2 < entry.contents.length; i2++) {
                            ...
                            subOptions.push({
                                ...
                                enabled: true,
                                callback: function () {
                                    const block = BlocklyWrapper.getMainWorkspace().newBlock(entry2.internalName);
                                    block.initSvg();
                                    block.render();
    ...
    function separatorWorkspace() {
        return separator("separatorWorkspace", BlocklyWrapper.ContextMenu.ScopeType.WORKSPACE);
    }
    function separatorBlock() {
        return separator("separatorBlock", BlocklyWrapper.ContextMenu.ScopeType.BLOCK);
    }
    ...
    function optionsWorkspace() {
        return createMenu("optionsWorkspace", "Options", BlocklyWrapper.ContextMenu.ScopeType.WORKSPACE);
    }
    function optionsBlock() {
        return createMenu("optionsBlock", "Options", BlocklyWrapper.ContextMenu.ScopeType.BLOCK);
    }
    ...
    function createSubMenuItem(id, scopeType, scope) {
        ...
        return {
            text: typeof data.displayText === "string"
                ? data.displayText
    ...
    function loadJson(data) {
        const workspace = BlocklyWrapper.getMainWorkspace();
        try {
            const variables = BlocklyWrapper.Xml.textToDom(data.variables ? data.variables : "<xml />");
            BlocklyWrapper.Xml.domToVariables(variables, workspace);
            BlocklyWrapper.Xml.domToWorkspace(BlocklyWrapper.Xml.textToDom(data.mainWorkspace), workspace);
            return true;
        }
        catch (e) {
    ...
    function loadXml(xmlText) {
        try {
            ...
            variableBlocks.forEach((e) => {
                ...
                if (objectType &&
                    variableName &&
                    !variables.find((v) => v.objectType === objectType &&
            ...
            BlocklyWrapper.Xml.domToVariables(variablesXml, BlocklyWrapper.getMainWorkspace());
            ...
            BlocklyWrapper.Xml.domToWorkspace(xmlDom, BlocklyWrapper.getMainWorkspace());
            return true;
        }
        catch (e) {
    ...
    function showContextMenuWithBack(options) {
        ...
        BlocklyWrapper.ContextMenu.show(lastContextMenu.e, []
            .concat({
            text: "< Back",
            enabled: true,
            callback: () => {
                const menu = contextMenuStack.splice(contextMenuStack.length - 1, 1);
                BlocklyWrapper.ContextMenu.show(lastContextMenu.e, menu[0], lastContextMenu.rtl);
            },
        }, {
            text: "---",
            enabled: false,
    ...
    function hookContextMenu() {
        ...
        workspacePrototype.showContextMenu = function (e) {
            ...
            lastContextMenu = {
                e: e,
                options: BlocklyWrapper.ContextMenu.getOptionsForWorkspace(this),
                rtl: this.RTL,
            };
            updateMouseCoords(e);
    ...
    function initializeBlocks(blockDefinitions) {
        ...
        blockLookup.push({
            ...
            displayName: titleCase(BlocklyWrapper.getTranslation("PYRITE_CONDITION")),
        });
        /*blockLookup.push({
            type: "condition",
        ...
        blockLookup.push({
            ...
            displayName: BlocklyWrapper.getTranslation("PYRITE_CONVENIENCE_ARRAYCONTAINS"),
        });
        blockLookup.push({
            type: "action",
            category: getCategory("CONVENIENCE"),
            internalName: "IndexOfArrayValue",
            displayName: BlocklyWrapper.getTranslation("PYRITE_CONVENIENCE_INDEXOFARRAYVALUE"),
        });
        blockLookup.push({
            type: "action",
            category: getCategory("CONVENIENCE"),
            internalName: "RemoveFromArray",
            displayName: BlocklyWrapper.getTranslation("PYRITE_CONVENIENCE_REMOVEFROMARRAY"),
        });
        //Blocks - Selection Lists
        const selectionLists = [
            ...new Set(blockDefinitions.selectionLists.map((e) => e.listType + "Item")),
        ];
        ...
        for (let index = 0; index < blockDefinitions.values.length; index++) {
            ...
            blockLookup.push({
                ...
                displayName: BlocklyWrapper.getTranslation(element.displayNameSID) ||
                    element.name,
            });
        }
        ...
        function getCategory(key) {
            ...
            return BlocklyWrapper.getTranslation("PYRITE_TOOLBOX_" + key.replace(" ", "_").toUpperCase());
        }
    }
    function initializeWorkspaceEvents(workspace) {
        ...
        workspace.addChangeListener(function (e) {
            if (e.type === BlocklyWrapper.Events.CLICK ||
            ...
            else if (e.type === BlocklyWrapper.Events.BLOCK_DRAG &&
                !e.isStart) {
                activeBlock = e.blockId;
            }
            else if (e.type === BlocklyWrapper.Events.MOVE &&
                e.newCoordinate &&
                e.oldCoordinate &&
                activeBlock) {
    ...
    function updateMouseCoords(event) {
        ...
        const mouseXY = BlocklyWrapper.Xml.mouseToSvg(event, mainWorkspace.getParentSvg(), mainWorkspace.getInverseScreenCTM());
...
BF2042Portal.Plugins = (function () {
    ...
    function loadPlugin(pluginData) {
        try {
            ...
            scriptElement.setAttribute("type", pluginData.manifest.loadAsModule ? "module" : "text/javascript");
